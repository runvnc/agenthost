...
impl LocalChat {
    ...
    pub fn generate_text(&mut self) -> anyhow::Result<()> {
    use tracing_chrome::ChromeLayerBuilder;
    use tracing_subscriber::prelude::*;

    let args = &self.args;
    let temperature = if self.args.temperature == 0. {
        None
    } else {
    ...
    let mut model = match model_path.extension().and_then(|v| v.to_str()) {
        ...
        Some("ggml" | "bin") | Some(_) | None => {
            ...
            let default_gqa = match self.args.which {
                Which::L7b
                | Which::L13b
                | Which::L7bChat
                | Which::L13bChat
            ...
            ModelWeights::from_ggml(model, self.args.gqa.unwrap_or(default_gqa))?
        }
    };

    let tokenizer = self.args.tokenizer()?;
